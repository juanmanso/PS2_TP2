...
Acel_real = Acel;
Acel(:,2) = Acel_real(:,2) + As_x;
Acel(:,3) = Acel_real(:,3) + As_y;
Acel_s_x = Acel(:,2) - Acel_real(:,2);
Acel_s_y = Acel(:,3) - Acel_real(:,3);
...
aux_Vx_O = -sin(O) * (A_x - A_x_s) - cos(O) * (A_y - A_y_s);
aux_Vx_Axs = - cos(O);
aux_Vx_Ays = sin(O);
aux_Vy_O =  cos(O) * (A_x - A_x_s) - sin(O) * (A_y - A_y_s);
aux_Vy_Axs = -sin(O);
aux_Vy_Ays = -cos(O);

aux_mat = [ aux_Vx_O, aux_Vx_Axs, aux_Vx_Ays;
            aux_Vy_O, aux_Vy_Axs, aux_Vy_Ays;   ];

A_t = [ eye(2), T_s_D * eye(2), zeros(2,3);
        zeros(2), eye(2), T_s_D * aux_mat;
        zeros(3,2), zeros(3,2), eye(3);];
    
B_t = 0;

C_t = [ eye(2), zeros(2), zeros(2,3);
        zeros(2), eye(2), zeros(2,3);  ];
...
P_0 = diag( [ dP_i^2, dP_i^2, dV_i^2, dV_i^2, (dTheta_i)^2, dA_i^2, dA_i^2] );
...
for i = 1:samples_size
    ...
    A = [ eye(2), T_s_D * eye(2), zeros(2,3);
            zeros(2), eye(2), T_s_D * aux_mat;
            zeros(3,2), zeros(3,2), eye(3);];

    B = 0;

    C = [ eye(2), zeros(2), zeros(2,3);
            zeros(2), eye(2), zeros(2,3);  ];
    
    X_pre = [   P_x + T_s_D * V_x;
                P_y + T_s_D * V_y;
                V_x + T_s_D * ((A_x - A_x_s) * cos(O) - (A_y - A_y_s) * sin(O));
                V_y + T_s_D * ((A_x - A_x_s) * sin(O) + (A_y - A_y_s) * cos(O));
                O + T_s_D * w_t;
                A_x_s;
                A_y_s;  ];
